...
def wavespace():
    ...
    # Symlet
    wave_family += ["sym4", "sym5", "sym6", "sym7"]
...
def wavecoef(X, wave_family):
    ...
    for wname in wave_family:
        ...
        # pywt's structural maximum, and an empirical EDL cap.
        max_allowed = pywt.dwt_max_level(data_len=len(X), filter_len=wavelet.dec_len)
        trial_level = min(Nmax, max_allowed)
        trial_coeffs = pywt.wavedec(X, wavelet=wname, level=trial_level, mode="symmetric")
        trial_details = trial_coeffs[1:][::-1]
        edl_empirical = max(
            [j + 1 for j, d in enumerate(trial_details) if len(d) / wavelet.dec_len > 1.5],
            default=0,
        )
        level = min(Nmax, max_allowed, edl_empirical)
...
def optimal_wavelets(X, wave_family, nw=5):
    ...
    # Paper-consistent cap: only keep levels up to EDL (Rj > 1.5).
    # Coefficients beyond EDL are filter-dominated and must not affect
    # kappa or mu_sc selection.
    det_coef_edl = {}
    for wname, details in det_coef.items():
        edl = int(edl_info[wname]["effective_level"])
        if edl <= 0:
            continue
        det_coef_edl[wname] = details[:edl]

    s = sparsity(det_coef_edl)
    sc = sparsity_change(s)
    dl = decomlevel(sc, threshold=0.05)

    # Safety clamp in case any downstream rule proposes kappa above EDL.
    for wname in list(dl.keys()):
        dl[wname] = int(min(int(dl[wname]), int(edl_info[wname]["effective_level"])))
